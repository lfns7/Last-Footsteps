...
void UUI_PlayerVictory::NativeConstruct()
{
    // 게임 인스턴스 캐스트
    ...
    if (RestartButton)
    {
        RestartButton->OnClicked.AddDynamic(this, &UUI_PlayerVictory::OnRestartClicked);
    }

    // 메인메뉴 호출
    if (MenuButton)
    {
        MenuButton->OnClicked.AddDynamic(this, &UUI_PlayerVictory::OnMenuClicked);
    }
    ...
    if (ResultText && GM && GI_GameInstance)
    {
        ...
        if (GI_GameInstance->GI_BestRecord <= GM->Time || GI_GameInstance->GI_BestScore <= Score)
        {
        ...
        {
            NewRecord->SetVisibility(ESlateVisibility::Visible);
            GI_GameInstance->GI_BestScore = Score * 1.5f;
            GI_GameInstance->SaveData();
            bCheckSRecord = true;
        }
        ...
        FText ResultCurrentScore = FText::Format(FText::FromString(TEXT(" / {0}점")), FText::AsNumber(Score * 1.5f));
...
void UUI_PlayerVictory::OnRestartClicked()
{
...
void UUI_PlayerVictory::OnMenuClicked()
{
...
void UUI_PlayerVictory::PlayVictorySound()
{
    USoundBase* SoundFinder = LoadObject<USoundBase>(nullptr, TEXT("/Game/Resources/Sounds/Use/Victory/Victory.Victory"));
    if (SoundFinder)
    {
...
void UUI_PlayerVictory::PlayStartAnimation()
{
    if (VictoryAnimation)
    {
